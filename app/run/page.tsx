...
export default function RunPage() {
  ...
  // ✅ FIX: useRef দিয়ে seconds track করা — GPS callback এ stale closure হবে না
  ...
  const startTracking = (selectedActivity: ActivityType) => {
    ...
    // ✅ FIX: secondsRef ও update হচ্ছে timer এ
      ...
      (pos) => {
        ...
        if (lastPos.current) {
          ...
          if (d >= minDist && speedKmh <= maxSpeed && speedKmh > 0.5) {
            ...
            // ✅ FIX: secondsRef.current দিয়ে accurate calories
            const cal = calcCalories(selectedActivity, userWeight, secondsRef.current);
            setCalories(cal);

            // ✅ FIX: secondsRef.current দিয়ে accurate steps
            const pace = totalDistance.current > 0
              ? (secondsRef.current / 60) / totalDistance.current
              : 0;
  ...
  const handleFinish = async () => {
    ...
    // ✅ FIX: secondsRef.current দিয়ে accurate pace
    const pace = distance > 0.01 ? ((secondsRef.current / 60) / distance).toFixed(2) : "0.00";
    ...
    router.push(
      `/result?km=${distance.toFixed(2)}&time=${formatTime2(secondsRef.current)}&pace=${pace}&calories=${calories}&steps=${steps}&activity=${activity}`
    );
  };
  ...
  // ✅ FIX: secondsRef.current দিয়ে accurate pace display
  const paceValue = distance > 0.01 && secondsRef.current > 0
    ? Math.round((secondsRef.current / 60) / distance * 10) / 10
    : 0;
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#0D1117", fontFamily: "'Archivo Black', sans-serif", display: "flex", flexDirection: "column", alignItems: "center", padding: "0 24px" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: "32px" }}>
        ...
        {currentSpeed > 0 && (
          <p style={{ color: "#4F6EF7", fontSize: "13px", fontFamily: "system-ui", marginTop: "4px" }}>{currentSpeed} km/h</p>
        )}
      </div>
      ...
      <div style={{ position: "fixed", bottom: "40px", left: "24px", right: "24px" }}>
        <button onClick={handleFinish}
          style={{ width: "100%", padding: "18px", borderRadius: "30px", background: "linear-gradient(135deg, #EF4444, #DC2626)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "12px", boxShadow: "0 8px 32px rgba(239,68,68,0.4)" }}>
